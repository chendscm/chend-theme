...
;;
(defcustom chend-themes-enable-bold t
  "If nil, bold will be disabled across all faces."
  :group 'chend-themes
  :type 'boolean)

(defcustom chend-themes-enable-italic t
  "If nil, italics will be disabled across all faces."
  :group 'chend-themes
  :type 'boolean)

(defvar chend-themes--colors nil)
(defvar chend--min-colors '(257 256 16))
(defvar chend--quoted-p nil)
(defvar chend-themes--faces nil)

(defun chend-themes--colors-p (item)
  (declare (pure t) (side-effect-free t))
  (when item
    (cond ((listp item)
           (let ((car (car item)))
             (cond ((memq car '(quote chend-color)) nil)

                   ((memq car '(backquote \`))
                    (let ((chend--quoted-p t))
                      (chend-themes--colors-p (cdr item))))

                   ((eq car '\,)
                    (let (chend--quoted-p)
                      (chend-themes--colors-p (cdr item))))

                   ((or (chend-themes--colors-p car)
                        (chend-themes--colors-p (cdr-safe item)))))))

          ((and (symbolp item)
                (not (keywordp item))
                (not chend--quoted-p)
                (not (equal (substring (symbol-name item) 0 1) "-"))
                (assq item chend-themes--colors))))))

(defun chend-themes--apply-faces (new-faces &optional default-faces)
  (declare (pure t) (side-effect-free t))
  (let ((default-faces (or default-faces chend-themes-base-faces))
    ...
    (let (results)
      (maphash (lambda (face plist)
                 (when (keywordp (car plist))
                   ...
                   (dolist (prop (append (unless chend-themes-enable-bold   '(:weight normal :bold nil))
                                         (unless chend-themes-enable-italic '(:slant normal :italic nil))))
...
(defun chend-themes--colorize (item type)
  (declare (pure t) (side-effect-free t))
  (when item
    (let ((chend--quoted-p chend--quoted-p))
      (cond ((listp item)
             (cond ((memq (car item) '(quote chend-color))
                    item)
                   ((eq (car item) 'chend-ref)
                    (chend-themes--colorize
                     (apply #'chend-ref (cdr item)) type))
                   ((let* ((item (append item nil))
                      ...
                      (cons car
                            (cl-loop
                             for i in (cdr item)
                             collect (chend-themes--colorize i type)))))))
            ...
            ((and (symbolp item)
                  ...
                  (not (equal (substring (symbol-name item) 0 1) "-"))
                  (assq item chend-themes--colors))
             `(chend-color ',item ',type))

            (item)))))

(defun chend-themes--build-face (face)
  (declare (pure t) (side-effect-free t))
  `(list
    ',(car face)
    ,(let ((face-body (cdr face)))
       (cond ((keywordp (car face-body))
              (let ((real-attrs face-body)
                    defs)
                (if (chend-themes--colors-p real-attrs)
                    (dolist (cl chend--min-colors `(list ,@(nreverse defs)))
                      (push `(list '((class color) (min-colors ,cl))
                                   (list ,@(chend-themes--colorize real-attrs cl)))
                            defs))
                  ...
                  (cond ((eq (car attrs) '&all)
                        ...
                        ((memq (car attrs) '(&dark &light))
                         (let ((bg (if (eq (car attrs) '&dark) 'dark 'light))
                           ...
                           (cond ((chend-themes--colors-p real-attrs)
                                  (dolist (cl chend--min-colors)
...
;;;###autoload
(defun chend-blend (color1 color2 alpha)
  "Blend two colors (hexidecimal strings) together by a coefficient ALPHA (a
float between 0 and 1)"
  (when (and color1 color2)
    (cond ((and color1 color2 (symbolp color1) (symbolp color2))
           (chend-blend (chend-color color1) (chend-color color2) alpha))

          ((or (listp color1) (listp color2))
           (cl-loop for x in color1
                    when (if (listp color2) (pop color2) color2)
                    collect (chend-blend x it alpha)))

          ((and (string-prefix-p "#" color1) (string-prefix-p "#" color2))
           (apply (lambda (r g b) (format "#%02x%02x%02x" (* r 255) (* g 255) (* b 255)))
                  (cl-loop for it    in (chend-name-to-rgb color1)
                           for other in (chend-name-to-rgb color2)
                           collect (+ (* alpha it) (* other (- 1 alpha))))))

          (color1))))

;;;###autoload
(defun chend-darken (color alpha)
  "Darken a COLOR (a hexidecimal string) by a coefficient ALPHA (a float between
0 and 1)."
  (cond ((and color (symbolp color))
         (chend-darken (chend-color color) alpha))

        ((listp color)
         (cl-loop for c in color collect (chend-darken c alpha)))

        ((chend-blend color "#000000" (- 1 alpha)))))

;;;###autoload
(defun chend-lighten (color alpha)
  "Brighten a COLOR (a hexidecimal string) by a coefficient ALPHA (a float
between 0 and 1)."
  (cond ((and color (symbolp color))
         (chend-lighten (chend-color color) alpha))

        ((listp color)
         (cl-loop for c in color collect (chend-lighten c alpha)))

        ((chend-blend color "#FFFFFF" (- 1 alpha)))))

;;;###autoload
(defun chend-color (name &optional type)
  "Retrieve a specific color named NAME (a symbol) from the current theme."
  (let ((colors (if (listp name)
                    name
                  (cdr-safe (assq name chend-themes--colors)))))
    (and colors
         (cond ((listp colors)
...
;;;###autoload
(defun chend-ref (face prop &optional class)
  "TODO"
  (let ((spec (or (cdr (assq face chend-themes--faces))
...
(defun chend-themes-prepare-facelist (custom-faces)
  "Return an alist of face definitions for `custom-theme-set-faces'.

Faces in EXTRA-FACES override the default faces."
  (declare (pure t) (side-effect-free t))
  (setq chend-themes--faces (chend-themes--apply-faces custom-faces))
  (mapcar #'chend-themes--build-face chend-themes--faces))

(defun chend-themes-prepare-varlist (vars)
  "Return an alist of variable definitions for `custom-theme-set-variables'.

Variables in EXTRA-VARS override the default ones."
  (declare (pure t) (side-effect-free t))
  (cl-loop for (var val) in (append chend-themes-base-vars vars)
           collect `(list ',var ,val)))

;;;###autoload
(defun chend-themes-set-faces (theme &rest faces)
  "Customize THEME (a symbol) with FACES.

If THEME is nil, it applies to all themes you load. FACES is a list of Chend
theme face specs. These is a simplified spec. For example:

  (chend-themes-set-faces 'user
  ...
  (apply #'custom-theme-set-faces
         (or theme 'user)
         (eval
          `(let* ((bold   ,chend-themes-enable-bold)
                  (italic ,chend-themes-enable-italic)
                  ,@(cl-loop for (var . val) in chend-themes--colors
                             collect `(,var ',val)))
...
(defmacro def-chend-theme (name docstring defs &optional extra-faces extra-vars)
  ...
  (let ((chend-themes--colors defs))
    `(let* ((bold   chend-themes-enable-bold)
            (italic chend-themes-enable-italic)
            ,@defs)
       (setq chend-themes--colors
             (list ,@(cl-loop for (var val) in defs
                              collect `(cons ',var ,val))))
       (deftheme ,name ,docstring)
       (custom-theme-set-faces
        ',name ,@(chend-themes-prepare-facelist extra-faces))
       (custom-theme-set-variables
        ',name ,@(chend-themes-prepare-varlist extra-vars))
...
(defvar chend-themes-base-faces
  '(;; --- custom faces -----------------------
    (chend-modeline-error
     :background (chend-darken red 0.25)
     :foreground base0
     :distant-foreground base0)
    (chend-visual-bell :background error)
    ...
    (custom-saved             :foreground green :background (chend-blend green bg 0.2) :bold bold)
    (custom-comment           :foreground fg :background region)
    (custom-comment-tag       :foreground grey)
    (custom-modified          :foreground blue :background (chend-blend blue bg 0.2))
    ...
    (custom-invalid           :foreground red :background (chend-blend red bg 0.2))
    (custom-variable-obsolete :foreground grey :background bg)
    (custom-state             :foreground green :background (chend-blend green bg 0.2))
    ...
    ;; ediff
    (ediff-fine-diff-A    :background (chend-blend selection bg 0.7) :weight 'bold :extend t)
    (ediff-fine-diff-B    :inherit 'ediff-fine-diff-A)
    (ediff-fine-diff-C    :inherit 'ediff-fine-diff-A)
    (ediff-current-diff-A :background (chend-blend selection bg 0.3) :extend t)
    ...
    (message-header-cc         :inherit 'message-header-to :foreground (chend-darken highlight 0.15))
    ...
    ;; vterm
    (vterm               :foreground fg)
    (vterm-color-black   :background (chend-lighten base0 0.25)   :foreground base0)
    (vterm-color-red     :background (chend-lighten red 0.25)     :foreground red)
    (vterm-color-green   :background (chend-lighten green 0.25)   :foreground green)
    (vterm-color-yellow  :background (chend-lighten yellow 0.25)  :foreground yellow)
    (vterm-color-blue    :background (chend-lighten blue 0.25)    :foreground blue)
    (vterm-color-magenta :background (chend-lighten magenta 0.25) :foreground magenta)
    (vterm-color-cyan    :background (chend-lighten cyan 0.25)    :foreground cyan)
    (vterm-color-white   :background (chend-lighten base8 0.25)   :foreground base8)
    ...
    (all-the-icons-lred     :foreground (chend-lighten red 0.3))
    (all-the-icons-dred     :foreground (chend-darken red 0.3))
    (all-the-icons-green    :foreground green)
    (all-the-icons-lgreen   :foreground (chend-lighten green 0.3))
    (all-the-icons-dgreen   :foreground (chend-darken green 0.3))
    (all-the-icons-yellow   :foreground yellow)
    (all-the-icons-lyellow  :foreground (chend-lighten yellow 0.3))
    (all-the-icons-dyellow  :foreground (chend-darken yellow 0.3))
    (all-the-icons-blue     :foreground blue)
    (all-the-icons-blue-alt :foreground teal)
    (all-the-icons-lblue    :foreground (chend-lighten blue 0.3))
    (all-the-icons-dblue    :foreground dark-blue)
    (all-the-icons-maroon   :foreground magenta)
    (all-the-icons-lmaroon  :foreground (chend-lighten magenta 0.3))
    (all-the-icons-dmaroon  :foreground (chend-darken magenta 0.3))
    (all-the-icons-purple   :foreground violet)
    (all-the-icons-lpurple  :foreground (chend-lighten violet 0.3))
    (all-the-icons-dpurple  :foreground (chend-darken violet 0.3))
    (all-the-icons-cyan     :foreground cyan)
    (all-the-icons-cyan-alt :foreground cyan)
    (all-the-icons-lcyan    :foreground (chend-lighten cyan 0.3))
    (all-the-icons-dcyan    :foreground dark-cyan)
    (all-the-icons-pink     :foreground (chend-lighten red 0.35))
    (all-the-icons-lpink    :foreground (chend-lighten red 0.55))
    (all-the-icons-dpink    :foreground red)
    (all-the-icons-silver   :foreground (chend-lighten grey 0.45))
    (all-the-icons-lsilver  :foreground (chend-lighten grey 0.7))
    (all-the-icons-dsilver  :foreground (chend-lighten grey 0.1))
    ...
    (avy-lead-face-0
     ...
     (&dark  :background (chend-lighten highlight 0.3))
     (&light :background (chend-darken highlight 0.3)))
    (avy-lead-face-1
     (&all   :inherit 'avy-lead-face)
     (&dark  :background (chend-lighten highlight 0.6))
     (&light :background (chend-darken highlight 0.6)))
    (avy-lead-face-2
     (&all   :inherit 'avy-lead-face)
     (&dark  :background (chend-lighten highlight 0.9))
     (&light :background (chend-darken highlight 0.9)))
    ...
    (cfw:face-header             :foreground (chend-blend blue bg 0.8) :weight 'bold)
    (cfw:face-sunday             :foreground (chend-blend red bg 0.8)  :weight 'bold)
    (cfw:face-saturday           :foreground (chend-blend red bg 0.8)  :weight 'bold)
    ...
    (company-tooltip-search-selection :background (chend-darken selection 0.25))
    ...
    (diff-context
     (&dark  :foreground (chend-darken fg 0.12))
     (&light :foreground (chend-lighten fg 0.12)))
    ...
    ;; dired-subtree
    (dired-subtree-depth-1-face :background (chend-darken bg-alt 0.02))
    (dired-subtree-depth-2-face :background (chend-darken bg-alt 0.04))
    (dired-subtree-depth-3-face :background (chend-darken bg-alt 0.06))
    (dired-subtree-depth-4-face :background (chend-darken bg-alt 0.08))
    (dired-subtree-depth-5-face :background (chend-darken bg-alt 0.10))
    (dired-subtree-depth-6-face :background (chend-darken bg-alt 0.12))

    ;; diredfl
    (diredfl-autofile-name          :foreground base4)
    (diredfl-compressed-file-name   :foreground yellow)
    (diredfl-compressed-file-suffix :foreground (chend-blend orange bg 0.6))
    (diredfl-date-time              :foreground cyan :weight 'light)
    (diredfl-deletion               :foreground red :background (chend-blend red bg 0.2) :weight 'bold)
    (diredfl-deletion-file-name     :foreground red :background (chend-blend red bg 0.2))
    ...
    (diredfl-file-suffix            :foreground (chend-blend fg bg 0.6))
    (diredfl-flag-mark              :foreground yellow :background (chend-blend yellow bg 0.2) :weight 'bold)
    (diredfl-flag-mark-line         :background (chend-blend yellow bg 0.1))
    ...
    ;; chend-modeline
    (chend-modeline-eldoc-bar :background green)
    (chend-modeline-bar-inactive :background nil) ; transparent
    ...
    (erc-header-line :background (chend-darken bg-alt 0.15) :foreground highlight)
    ...
    (gnus-summary-high-ancient   :foreground (chend-lighten base5 0.2) :inherit 'italic)
    (gnus-summary-high-read      :foreground (chend-lighten fg 0.2))
    (gnus-summary-high-ticked    :foreground (chend-lighten magenta 0.2))
    (gnus-summary-high-unread    :foreground (chend-lighten green 0.2))
    (gnus-summary-low-ancient    :foreground (chend-darken base5 0.2) :inherit 'italic)
    (gnus-summary-low-read       :foreground (chend-darken fg 0.2))
    (gnus-summary-low-ticked     :foreground (chend-darken magenta 0.2))
    (gnus-summary-low-unread     :foreground (chend-darken green 0.2))
    ...
    ;; hideshow
    (+chend-folded-face :inherit 'font-lock-comment-face
                          :weight 'light
                          :background (chend-darken bg 0.125))
    ...
    (ivy-minibuffer-match-face-1
     :background nil
     :foreground (chend-lighten grey 0.14)
     :weight 'light)
    ...
    ;; ivy-posframe
    (ivy-posframe :background (chend-darken bg-alt 0.2))
    ...
    (chend-neotree-dir-face :foreground highlight)
    (chend-neotree-file-face :foreground base8)
    (chend-neotree-hidden-file-face :foreground comments)
    (chend-neotree-text-file-face :foreground fg)
    (chend-neotree-data-file-face :foreground violet)
    (chend-neotree-media-file-face :inherit 'chend-neotree-hidden-file-face)
    ...
    (lsp-ui-peek-header :foreground fg :background (chend-lighten bg 0.1) :bold bold)
    (lsp-ui-peek-selection :foreground bg :background blue :bold bold)
    (lsp-ui-peek-list :background (chend-darken bg 0.1))
    (lsp-ui-peek-peek :background (chend-darken bg 0.1))
    ...
    (magit-diff-added             :foreground (chend-darken green 0.2)  :background (chend-blend green bg 0.1) :extend t)
    (magit-diff-added-highlight   :foreground green                    :background (chend-blend green bg 0.2) :weight 'bold :extend t)
    (magit-diff-base              :foreground (chend-darken orange 0.2) :background (chend-blend orange bg 0.1) :extend t)
    (magit-diff-base-highlight    :foreground orange                   :background (chend-blend orange bg 0.2) :weight 'bold :extend t)
    (magit-diff-context           :foreground (chend-darken fg 0.4) :background bg :extend t)
    ...
    (magit-diff-hunk-heading           :foreground bg                    :background (chend-blend violet bg 0.3) :extend t)
    (magit-diff-hunk-heading-highlight :foreground bg                    :background violet :weight 'bold :extend t)
    (magit-diff-removed                :foreground (chend-darken red 0.2) :background (chend-blend red base3 0.1) :extend t)
    (magit-diff-removed-highlight      :foreground red                   :background (chend-blend red base3 0.2) :weight 'bold :extend t)
    ...
    (objed-hl        :inherit 'region :background (chend-blend region bg 0.5))
    ...
    (powerline-active1   :inherit 'mode-line :background (chend-lighten 'bg 0.025))
    (powerline-active2   :inherit 'mode-line :foreground base8 :background (chend-lighten 'bg 0.08))
    (powerline-inactive0 :inherit 'mode-line-inactive :background base2)
    (powerline-inactive1 :inherit 'mode-line-inactive :background (chend-lighten 'base2 0.02))
    (powerline-inactive2 :inherit 'mode-line-inactive :background (chend-lighten 'base2 0.04))
    ...
    ;; smerge-tool
    (smerge-lower :background (chend-blend green bg 0.2))
    (smerge-upper :background (chend-blend red base3 0.2))
    (smerge-base  :background (chend-blend blue bg 0.2))
    ...
    (tldr-introduction     :foreground (chend-blend blue bg 0.8) :weight 'semi-bold)
    ...
    ;; twittering-mode
    (twitter-divider                    ; custom face in Chend Emacs
     (&light :underline `(:color ,(chend-lighten vertical-bar 0.2)))
     (&dark  :underline `(:color ,(chend-darken vertical-bar 0.2))))
    ...
    (font-latex-sectioning-3-face :foreground (chend-lighten blue 0.3)    :weight 'semi-bold)
    (font-latex-sectioning-4-face :foreground (chend-lighten magenta 0.3) :weight 'semi-bold)
    (font-latex-sectioning-5-face :foreground (chend-lighten violet 0.3)  :weight 'semi-bold)
    ...
    (notmuch-search-flagged-face             :foreground (chend-blend red base4 0.5))
    ...
    (outline-4 :foreground (chend-lighten blue 0.25)    :weight 'bold :extend t)
    (outline-5 :foreground (chend-lighten magenta 0.25) :weight 'bold :extend t)
    (outline-6 :foreground (chend-lighten blue 0.5)     :weight 'bold :extend t)
    (outline-7 :foreground (chend-lighten magenta 0.5)  :weight 'bold :extend t)
    (outline-8 :foreground (chend-lighten blue 0.8)     :weight 'bold :extend t)
    ...
    (org-agenda-date-today    :foreground (chend-lighten violet 0.4)   :weight 'ultra-bold)
    (org-agenda-date-weekend  :foreground (chend-darken violet 0.4)  :weight 'ultra-bold)
    (org-agenda-structure     :foreground fg :weight 'ultra-bold)
    (org-agenda-clocking      :background (chend-blend blue bg 0.2))
    (org-upcoming-deadline         :foreground (chend-blend fg bg 0.8))
    (org-upcoming-distant-deadline :foreground (chend-blend fg bg 0.5))
    ...
    (org-habit-ready-face          :weight 'bold :background (chend-blend blue bg-alt 0.5))
    (org-habit-ready-future-face   :weight 'bold :background (chend-blend blue bg-alt 0.3))
    (org-habit-alert-face          :weight 'bold :background (chend-blend yellow bg-alt 0.5))
    (org-habit-alert-future-face   :weight 'bold :background (chend-blend yellow bg-alt 0.3))
    (org-habit-overdue-face        :weight 'bold :background (chend-blend red bg-alt 0.5))
    (org-habit-overdue-future-face :weight 'bold :background (chend-blend red bg-alt 0.3))
    ...
    (telephone-line-evil-insert :background (chend-blend green bg 0.5) :weight 'bold)
    (telephone-line-evil-normal :background (chend-blend red bg 0.5) :weight 'bold)
    (telephone-line-evil-visual :background (chend-blend orange bg 0.5) :weight 'bold)
    (telephone-line-evil-replace :background (chend-color bg-alt) :weight 'bold)
    (telephone-line-evil-motion :background (chend-blend blue bg 0.5) :weight 'bold)
    (telephone-line-evil-operator :background (chend-blend violet bg 0.5) :weight 'bold)
    (telephone-line-evil-emacs :background (chend-blend magenta bg 0.5) :weight 'bold)

    ;; typescript-mode
    (typescript-jsdoc-tag :foreground doc-comments)
    (typescript-jsdoc-type :foreground (chend-darken doc-comments 0.15))
    (typescript-jsdoc-value :foreground (chend-lighten doc-comments 0.15))
...
(defvar chend-themes-base-vars
  '((ansi-color-names-vector
     (vconcat (mapcar #'chend-color '(bg red green yellow blue magenta cyan fg))))
    (rustic-ansi-faces
     (vconcat (mapcar #'chend-color '(bg red green yellow blue magenta cyan fg))))

    (fci-rule-color (chend-color 'base5))

    (jdee-db-spec-breakpoint-face-colors `(cons ,(chend-color 'base0) ,(chend-color 'grey)))
    (jdee-db-requested-breakpoint-face-colors `(cons ,(chend-color 'base0) ,(chend-color 'green)))
    (jdee-db-active-breakpoint-face-colors `(cons ,(chend-color 'base0) ,(chend-color 'highlight)))

    (objed-cursor-color (chend-color 'red))

    (pdf-view-midnight-colors `(cons ,(chend-color 'fg) ,(chend-color 'bg)))

    (vc-annotate-color-map
     `(list (cons 20  ,(chend-color 'green))
            (cons 40  ,(chend-blend 'yellow 'green (/ 1.0 3)))
            (cons 60  ,(chend-blend 'yellow 'green (/ 2.0 3)))
            (cons 80  ,(chend-color 'yellow))
            (cons 100 ,(chend-blend 'orange 'yellow (/ 1.0 3)))
            (cons 120 ,(chend-blend 'orange 'yellow (/ 2.0 3)))
            (cons 140 ,(chend-color 'orange))
            (cons 160 ,(chend-blend 'magenta 'orange (/ 1.0 3)))
            (cons 180 ,(chend-blend 'magenta 'orange (/ 2.0 3)))
            (cons 200 ,(chend-color 'magenta))
            (cons 220 ,(chend-blend 'red 'magenta (/ 1.0 3)))
            (cons 240 ,(chend-blend 'red 'magenta (/ 2.0 3)))
            (cons 260 ,(chend-color 'red))
            (cons 280 ,(chend-blend 'grey 'red (/ 1.0 4)))
            (cons 300 ,(chend-blend 'grey 'red (/ 2.0 4)))
            (cons 320 ,(chend-blend 'grey 'red (/ 3.0 4)))
            (cons 340 ,(chend-color 'base5))
            (cons 360 ,(chend-color 'base5))))
    (vc-annotate-very-old-color nil)
    (vc-annotate-background (chend-color 'bg)))
  "TODO")

(defvar chend-theme-status "init")

(defun chend-theme-load-light ()
  (mapc #'disable-theme custom-enabled-themes)
  (load-theme 'chend-light t)
  (setq chend-theme-status "light"))

(defun chend-theme-load-dark ()
  (mapc #'disable-theme custom-enabled-themes)
  (load-theme 'chend-dark t)
  (setq chend-theme-status "dark"))

(defun chend-theme-toggle ()
  (interactive)
